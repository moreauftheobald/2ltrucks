...
//if (g)
{
	...
	//g.timescaleStartDateMode("month");
	...
	for ($tcursor=0; $tcursor < $tnums; $tcursor++)
	{
		...
		if (empty($old_project_id) || $old_project_id != $t['task_project_id'])
		{
			...
			$tmpt = array(
				...
				'task_start_date'=>$projecttmp->date_start, 'task_end_date'=>$projecttmp->date_end,
				...
				'task_planned_workload'=>0, 'task_open'=>1
			);
...
function constructGanttLine($tarr, $task, $task_dependencies, $level = 0, $project_id = null)
{
    ...
    */
    ...
    $dependency =  $task['task_depends_alternate_id'];//$task['task_dependency'];
    ...
    $s.= "g.AddTaskItem(new JSGantt.TaskItem('".$taskid."', '".dol_escape_js(trim($name))."', '".$start_date."', '".$end_date."', '".$css."', '".$link."', ".$task['task_milestone'].", '".dol_escape_js($resources)."', ".($percent >= 0 ? $percent : 0).", ".$line_is_auto_group.", '".$parent."', " . $open . ", '".$dependency."', '".(empty($task["task_is_group"]) ? (($percent >= 0 && $percent != '') ? $percent.'%' : '') : '')."', '".dol_escape_js($note)."', g));";
    echo $s;
}
