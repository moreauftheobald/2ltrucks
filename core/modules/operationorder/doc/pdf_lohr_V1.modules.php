 ...
 */
class pdf_lohr_V1 extends ModelePDFOperationOrder
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = 'lohr_V1';
		$this->description = "Check list LOHR V1";
	...
	function add_doc($object,$outputlangs,$pdf,$productref,$docname)
	{
		...
		if(!empty($object->array_options['options_doc_ans'])){
		    ...
		    $docansraw = unserialize(html_entity_decode($object->array_options['options_doc_ans']));
			
			$docname = $docansraw[$productref];
			
			
			$docans = $docansraw['ans'][$docname];
			...
			$res = $product->fetch($docansraw[$docname]['ref']);
			if($res>0) {
			...
			//fill pdf with recorded data
			Foreach ($docans as $ans){
				$pdf->SetPage($lastpage+$ans[4]);
				$pdf->SetFont('','', $default_font_size+$ans[5]);
				$pdf->SetXY($ans[1], $ans[2]);
				$pdf->MultiCell($ans[3], 0, $ans[0],0,'L');
			}
