...
if ($action == 'save_multicompany_shared_conf')
{
	...
	{
		$multicompanypriceshare[$entity->id] = GETPOST('multicompany-supplier_order_'.$entity->id, 'array');
...
dol_fiche_head($head, 'multicompanySharing', $langs->trans("supplier_order"), -1, "supplier_order");
	...
	if (is_array($dao->entities))
	{
		foreach ($dao->entities as $entitie)
		{
			if (intval($conf->entity) === 1 || intval($conf->entity) === intval($entitie->id))
			{
				...
				print '</td>';
				print '<td align="center" >';
				print _multiselect_entities('multicompany-supplier_order_'.$entitie->id, $entitie, '', $element);
				print '</td>';
				print '</tr>';
			}
		}
