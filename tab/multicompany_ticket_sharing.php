...
dol_include_once('/core/lib/ticket.lib.php');
...
if ($action == 'save_multicompany_shared_conf')
{
	...
	{
		$multicompanypriceshare[$entity->id] = GETPOST('multicompany-ticket_'.$entity->id, 'array');
...
dol_fiche_head($head, 'multicompanySharing', $langs->trans("Tickets"), -1, "ticket");
...
if (!empty($conf->multicompany->enabled) && !empty($conf->global->MULTICOMPANY_SHARINGS_ENABLED))
{
	...
	print '<td align="left" >';
	print $langs->trans("ActivateticketSharing");
	print '</td>';
	...
	if (is_array($dao->entities))
	{
		foreach ($dao->entities as $entitie)
		{
			if (intval($conf->entity) === 1 || intval($conf->entity) === intval($entitie->id))
			{
				...
				print '</td>';
				print '<td align="center" >';
				print _multiselect_entities('multicompany-ticket_'.$entitie->id, $entitie, '', $element);
				print '</td>';
				print '</tr>';
			}
		}
